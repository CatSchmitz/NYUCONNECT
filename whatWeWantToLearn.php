...
<title> NYU Connect | What We Want To Learn </title>
...
<body>
	...
	//if there are no other errors, insert shout into database. Otherwise display errors 
		...
		if (sizeof($error) == 0)
		{
			...
			//insert post
			$sql = "INSERT INTO whatToLearn (text_added, date_added) VALUES ('$post', NOW())";
	...
	?>
    <p class="daily_heading"> What We Want To Learn </p> 
    <form method="post" action="whatWeWantToLearn.php"> 
    	
        	<textarea name="text_added" class="user_input_forms" placeholder="What do you want to learn in this course? Tell us what you would like to explore more."></textarea>
    ...
    <?php
		$sql = "SELECT text_added, DATE_FORMAT(date_added,'%M %d, %Y') AS formatted_date FROM whatToLearn ORDER BY date_added DESC";
		$result = mysqli_query($db, $sql) or die('Query failed: ' . mysqli_error($db));
